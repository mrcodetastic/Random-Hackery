...
void CC1101::configureGPIO() {
	
	if (debug_level >= 1) {
		Serial.println("Configuring GPIOs.");
	}
	
	// Need to have a slave select pin...
	assert( _cs != -1);	
	pinMode(_cs, OUTPUT);                  
	...
	if (CC1101_GDO0_interrupt_pin != -1)
	{
...
*/
bool CC1101::begin(CFREQ freq, uint8_t channr, uint8_t addr, uint8_t clk_pin, uint8_t miso_pin, uint8_t mosi_pin, uint8_t slave_select_pin)
{
  // spi config
  _sclk = (gpio_num_t) clk_pin;
  _miso = (gpio_num_t) miso_pin;
  _mosi = (gpio_num_t) mosi_pin;
  _cs   = slave_select_pin;

  // cc1101 config
  carrierFreq = freq;
  dataRate    = KBPS_38;
  
  //CC1101_GDO0_interrupt_pin = gd0_pin;
...
*/
 ...
 // setTxState(); 
  ...
  tryCount = 0;
  while ((readStatusRegSafe(CC1101_TXBYTES) & 0x7F) != 0) {  if (tryCount++ > 5) break; }
