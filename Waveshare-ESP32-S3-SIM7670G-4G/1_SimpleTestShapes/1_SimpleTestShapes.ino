...
void setup() {
  ...
  // Potential waveshare pins
  mxconfig.gpio.r1 = 4;
  mxconfig.gpio.g1 = 5;  
  mxconfig.gpio.b1 = 6;    
  mxconfig.gpio.a = 46;      
  mxconfig.gpio.b = 15;     // used by camera, try another one if using camera  - 41?         
  mxconfig.gpio.c = 16;     // used by camera, try another one if using camera  - 40? 
  mxconfig.gpio.d = 7;      // used by camera, try another one if using camera  - 39?     
  mxconfig.gpio.clk = 8;    // used by camera, try another one if using camera  - 45?         
  mxconfig.gpio.lat = 9;    // used by camera, try another one if using camera  - 33?            
  mxconfig.gpio.oe = 10;    // used by camera, try another one if using camera  - 42?              
  mxconfig.gpio.r2 = 11;    // used by camera, try another one if using camera  - 21?     
  mxconfig.gpio.g2 = 12;    // used by camera, try another one if using camera  - 1?     
  mxconfig.gpio.b2 = 13;    // used by camera, try another one if using camera  - Not enough spare pins unless you use a UART pin? No 4g, or usb serial comms?     
